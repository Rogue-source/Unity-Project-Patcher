...
namespace Rogue.UnityProjectPatcher.Editor {
    public class PatcherWindow: EditorWindow {
        ...
        private void OnEnable() {
            Rogue.UnityProjectPatcher.PatcherUtility.GetUserSettings();
        ...
        private static IEnumerable<PackageVersion> GetPackageVersions() {
            ...
            var toolGit = "https://github.com/Rogue-source/Unity-Project-Patcher";
            var gameWrapper = PatcherUtility.GetGameWrapperAttribute();
            
            var currentToolVersion = packages.FirstOrDefault(x => x.name == "com.rogue.unity-project-patcher")?.version;
            ...
            if (!string.IsNullOrEmpty(currentToolVersion) && PatcherUtility.TryFetchGitVersion(toolGit, out var toolVersion)) {
                ...
                if (currentToolVersion != toolVersion) {
                    Debug.LogWarning($"[com.rogue.unity-project-patcher] is <color=yellow>outdated</color>. Please open the patcher window to update to {toolVersion} from \"{toolGit}\". Current version: {currentToolVersion}.");
                } else {
                    //Debug.Log($"[com.rogue.unity-project-patcher] is up to date. Current version: {currentToolVersion}.");
                }
            } else {
                Debug.LogWarning($"Failed to fetch [com.rogue.unity-project-patcher] version from \"{toolGit}\".");
            }
